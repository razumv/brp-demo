...
test("Astryx shell persists a hydration-safe compact desktop rail with labelled controls", () => {
  ...
  assert.match(shellView, /usePersistedBooleanPreference\("brp-clone-ui-v1:astryx-sidebar-collapsed", false\)/);
  ...
  assert.match(shellView, /data-sidebar-collapsed=\{sidebarCollapsed \? "true" : "false"\}/);
  assert.match(shellView, /<SideNavItem[\s\S]*label=\{item\.label\}/);

  assert.match(shellClasses, /sideNavControls:/);
  assert.match(globalStyles, /\.brp-astryx-shell-side-nav\[data-sidebar-collapsed="true"\]/);
