...
function ProductMenu({id, sku, state, active}: {id: string; sku: string; state: CatalogPageViewProps; active: boolean}) {
  ...
  return <MoreMenu
    ...
    items={[
      {label: "Редагувати — вимкнено у read-only клоні", icon: <LockKeyhole size={14}/>, isDisabled: true},
      {label: "Видалити — вимкнено у read-only клоні", icon: <LockKeyhole size={14}/>, isDisabled: true},
    ]}
  />;
}
