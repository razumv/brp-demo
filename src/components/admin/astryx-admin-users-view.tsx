...
function UserCard({user, model}: {user: AdminUserRecord; model: AdminUsersModel}) {
  ...
  return (
    <Card padding={3} className={styles.userCard} data-record-id={user.id}>
...
export default function AstryxAdminUsersView({model, onReady}: Props) {
  ...
  return (
    <AstryxBrpUiProvider>
    <main className={styles.page} data-admin-users-renderer="astryx">
      ...
      <Card padding={3} className={styles.toolbar}><TextInput label="Пошук користувачів" isLabelHidden startIcon={<UsersRound size={15} />} value={model.query} onChange={model.setQuery} placeholder="Пошук за ім’ям, email, логіном або компанією..." hasClear width="100%" /><TabList aria-label="Стани користувачів" value={model.tab} onChange={(value) => model.setTab(value as typeof model.tab)} size="sm" layout="fill">{adminUserTabs.map((tab) => <Tab key={tab.id} value={tab.id} label={tab.label} icon={tab.id === "pending" ? <Clock3 size={13} /> : tab.id === "active" ? <CheckCircle2 size={13} /> : <CircleX size={13} />} />)}</TabList></Card>
      {model.visibleUsers.length ? <section className={styles.userGrid} aria-label="Активні користувачі">{model.visibleUsers.map((user) => <UserCard key={user.id} user={user} model={model} />)}</section> : <Card padding={6}><EmptyState title="Користувачів не знайдено" description={model.tab === "active" ? "Змініть пошуковий запит." : "У цій категорії немає користувачів."} /></Card>}
