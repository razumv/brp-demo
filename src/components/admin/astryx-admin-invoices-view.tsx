...
function Appendices(props: AdminInvoicesViewProps) {
  ...
  const columns = useMemo<TableColumn<AppendixRow>[]>(() => [
    {key: "name", header: "Додаток", width: pixel(150), renderCell: (row) => <Button label={row.name} variant="ghost" size="sm" isDisabled={!row.preview} tooltip={row.preview ? undefined : "Повний документ ще не завантажено"} onClick={() => props.onSelectedAppendixChange(row)} />},
