...
const READ_ONLY_TITLE = "Демо: операційна дія вимкнена";
    ...
    <div className={styles.modeStack} role="tabpanel" aria-label="Синхронізація складу">
      <section className={styles.panel}>
        <header className={styles.panelHeader}>
          <div><h2>Режим синхронізації</h2><p>Зафіксований read-only стан підключення</p></div>
      ...
      <section className={styles.panel}>
        ...
        <div className={`${styles.panelBody} ${styles.modeStack}`}>
          <button className={styles.modeButton} type="button" disabled title={READ_ONLY_TITLE}>
          ...
          <button className={`${styles.modeButton} ${styles.modeButtonActive}`} type="button" disabled title={READ_ONLY_TITLE}>
            <strong>OData Polling</strong>
            <span className="mt-1 block text-[11px] text-[var(--muted-foreground)]">Активний режим у зафіксованому стані; poller зупинено.</span>
          </button>
      ...
      <section className={styles.panel}>
        <header className={styles.panelHeader}><div><h2>Зіставлення та звіти</h2><p>Безпечні переходи до локальних read-only представлень</p></div></header>
...
function OneCTokensPanel() {
  return (
    <section className={styles.panel} role="tabpanel" aria-label="API-токени">
      <header className={styles.panelHeader}><div><h2>API-токени</h2><p>Токен навмисно повністю замаскований у локальному fixture</p></div></header>
        ...
        <table className={`${styles.table} ${styles.table860}`}>
          ...
          <tbody>
            <tr>
              ...
              <td><code>••••••••••••••••••••••••</code></td>
              <td><button type="button" className="button button-outline !min-h-7 px-2 text-[10px]" disabled title={READ_ONLY_TITLE}>Активний</button></td>
              ...
              <td>
                <div className="flex gap-1">
                  ...
                  <button type="button" className="icon-button" disabled title={READ_ONLY_TITLE} aria-label="Видалити токен"><Trash2 size={14} /></button>
                </div>
              </td>
            </tr>
          </tbody>
...
function OneCHistoryPanel() {
  ...
  return (
    <section className={styles.panel} role="tabpanel" aria-label="Історія експорту">
      <header className={styles.panelHeader}><div><h2>Історія експорту</h2><p>262 локальні детерміновані позиції · без мережевих запитів</p></div></header>
      <div className={styles.tableWrap}>
        <table className={`${styles.table} ${styles.table860}`}>
...
function OneCDocsPanel() {
  return (
    <section className={styles.panel} role="tabpanel" aria-label="Документація API">
      <header className={styles.panelHeader}><div><h2>Документація API</h2><p>Інертні приклади з тестовим доменом і placeholder-токеном</p></div></header>
      <div className={`${styles.panelBody} ${styles.docs}`}>
        ...
        <section>
          ...
          <code className={styles.codeBlock}>{`GET /api/webhook/1c/orders?limit=20\nAuthorization: Bearer <token>\n\n{\n  "items": [{ "order": "DEMO-001", "status": "pending" }]\n}`}</code>
        </section>
        <section>
          <h3>Phase 2 · Підтвердження</h3>
          <code className={styles.codeBlock}>{`POST /api/webhook/1c/orders/DEMO-001/confirm\nAuthorization: Bearer <token>\nContent-Type: application/json\n\n{ "externalReference": "DEMO-REFERENCE" }`}</code>
          <div className={`${styles.noticeRed} mt-3`}><AlertTriangle size={15} /><strong className="ml-2">Підтвердження змінює стан.</strong> Приклад показаний лише як текст і не може бути виконаний у клоні.</div>
        </section>
      </div>
    </section>
  );
  ...
  return (
    <main className="page">
      <div className={styles.stack}>
        ...
        {tab === "history" ? <OneCHistoryPanel /> : null}
...
export function AdminDealerMappingPage() {
  ...
  return (
    <main className="page">
      <div className={styles.stack}>
        <Link href="/admin/integrations/1c" className="flex w-fit items-center gap-2 text-[12px] text-[var(--muted-foreground)]"><ArrowLeft size={14} /> До інтеграції 1С</Link>
        <PageTitle icon={<Network size={22} />} title="Дилер → Контрагент 1С" description="Зіставлення дилерів із синтетичними read-only представленнями контрагентів" />
        <section className={styles.kpiGrid} aria-label="Показники зіставлення дилерів">
          <article className={styles.kpiCard}><small>Дилерів</small><strong>20</strong><span>Усього в локальному fixture</span></article>
        ...
        <section className={styles.panel}>
          <header className={styles.panelHeader}>
            <div><h2>Зіставлення дилерів</h2><p>Ідентифікатори й реєстраційні дані навмисно замінено синтетичними підписами</p></div>
                ...
                <tbody>
                  {visibleDealers.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td><div className="flex gap-1"><ReadOnlyButton className="!min-h-7 !px-2">Змінити</ReadOnlyButton><button type="button" className="icon-button" disabled title={READ_ONLY_TITLE} aria-label="Видалити зіставлення"><Trash2 size={14} /></button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.settingRow}>
          ...
          <div className={styles.settingControls}>
            ...
            {[30, 90, 180].map((value) => <button key={value} type="button" className="button button-outline" disabled title={READ_ONLY_TITLE}>{value} днів</button>)}
          </div>
        </div>
        <div className={`${styles.settingRow} border-t border-[var(--border)]`}>
          ...
          <div className={styles.settingControls}>
            ...
            {["30d", "90d", "180d", "360d", "720d"].map((value) => <button key={value} type="button" className="button button-outline" disabled title={READ_ONLY_TITLE}>{value}</button>)}
          </div>
        </div>
...
function BossOrderPositions({ order }: { order: BossWebOrderRow }) {
  return (
    <div className="grid gap-3 p-4">
      <div className="flex items-center justify-between gap-3"><strong>Позиції замовлення ({order.positions.length})</strong><span className={styles.summaryPill}>read-only</span></div>
      <div className={styles.tableWrap}>
        <table className={`${styles.table} ${styles.table820}`}>
  ...
  return (
    <main className="page">
      <div className={styles.stack}>
        ...
        {tab === "orders" ? <BossOrdersPanel /> : null}
