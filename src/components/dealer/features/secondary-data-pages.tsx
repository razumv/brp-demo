  ...
  return (
    <FeatureFrame feature="consignment">
      ...
      <Panel>
        ...
        <DealerDataToolbar search={{ value: query, onValueChange: setQuery, label: "Пошук консигнації", placeholder: "Номер, опис або дилер..." }} filters={{ label: "Фільтри", activeCount: Number(status !== "all"), open: filtersOpen, onOpenChange: setFiltersOpen, panelId: "consignment-filters", onClear: () => setStatus("all"), content: <BrpSelect label="Фільтр консигнації" value={status} onValueChange={(value) => setStatus(value as ConsignmentStatusFilter)} options={[{ value: "all", label: "Усі стани" }, { value: "available", label: "Доступно" }, { value: "reserved", label: "Резерв" }, { value: "requested", label: "Запит" }]} /> }} resultMeta={ukrainianCount(rows.length, ["позиція", "позиції", "позицій"])} />
...
export function InventoryPage() {
  ...
  return (
    <FeatureFrame feature="parts-inventory">
      ...
      <Panel><DealerDataToolbar search={{ value: query, onValueChange: setQuery, label: "Пошук складу", placeholder: "Номер або опис запчастини..." }} filters={{ label: "Фільтри", activeCount: Number(stock !== "all"), open: filtersOpen, onOpenChange: setFiltersOpen, panelId: "inventory-filters", onClear: () => setStock("all"), content: <BrpSelect label="Фільтр запасу" value={stock} onValueChange={(value) => setStock(value as StockFilter)} options={[{ value: "all", label: "Усі позиції" }, { value: "in-stock", label: "В наявності" }, { value: "low", label: "Низький залишок" }, { value: "out", label: "Немає в наявності" }]} /> }} resultMeta={ukrainianCount(rows.length, ["позиція", "позиції", "позицій"])} />{rows.length ? <TableRegion label="Склад дилера"><table className="data-table"><thead><tr><th>Запчастина</th><th>Залишок</th><th>Мінімум</th><th>Вартість</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.partNumber}</strong><small>{row.description}</small></td><td>{row.stock}</td><td>{row.reorderPoint}</td><td><strong>{formatMoney(row.stock * row.dealerPrice)}</strong></td></tr>)}</tbody></table></TableRegion> : <EmptyState icon={<Warehouse size={26} />} title="Позицій не знайдено" description="Змініть запит або фільтр запасу." />}</Panel>
    </FeatureFrame>
  );
}
