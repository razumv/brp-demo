...
|---|---|---|---|---|
| `/admin` | KPI, queue, shortcuts, recent orders | Overview/search/links | `clone-admin-overview-{desktop,mobile,tablet,dark}.png` | Legacy overview spec predates the strict evidence label |
...
| `/admin/ocean-freight` | Ocean, Ground, Dealer Equipment; table/cards/grouping; receipt/document previews; all seven Parts Receipt subtabs | Three tabs; grouped/ungrouped, table/cards; five dealer-equipment categories; BL-level receipt preview and seven independent receipt states | `clone-admin-ocean-freight-{desktop,mobile,tablet,dark}.png` | Network error and exact no-result copy not forced |
...
| `/admin/invoices` | Contracts, Appendices, Invoices, Cost | Four tabs; invoice All/Transit/Arrived; cost Active/Archive/Incomplete; previews | `clone-admin-invoices-{desktop,mobile}.png` | Source mobile main area is defective; clone keeps content accessible |
| `/admin/catalog` | Vehicles, Distributor Prices, Parts; all category tabs; filters/debug/history/pages | Three primary tabs; all 4 vehicle categories; all 4 distributor categories; parts filters/pages | `clone-admin-catalog-{desktop,mobile,tablet,dark}.png` | Durable route error not forced |
